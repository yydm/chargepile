...
function ajaxSuccess_setBusinessInfo(data) {
    ...
    $("#cdjssj").val(data.Rows[0].Enddt);
    $("#zdl").val(data.Rows[0].Charge_power); $("#fdl").val(data.Rows[0].Power_high); $("#pdl").val(data.Rows[0].Power_normal);
    $("#gdl").val(data.Rows[0].Power_low); $("#jdl").val(data.Rows[0].Power_tip);
    if (data.Rows[0].Charge_money == null) {
        $("#zje").val(0 + "元");
    } else {
        $("#zje").val(data.Rows[0].Charge_money + "元");
    }
    $("#kh").val(data.Rows[0].Card_no);
    if (data.Rows[0].Card_end_money == null) {
        $("#kye").val(0 + "元");
    } else {
        $("#kye").val(data.Rows[0].Card_end_money + "元");
    }
    if ((data.Rows[0].CheckOut_No != 0 && data.Rows[0].CheckOut_No != null) || 
        data.Rows[0].CreditCardDt != null) {
        $("#jszt").val("已结算");
    } else {
        $("#jszt").val("未结算");
    }
...
function ajaxSuccess_setChargePileInfo(data) {
    ...
    $("#tysj").val(tysj);
    $("#fdj").val(data.Rows[0].Fjg + "元");
    $("#pdj").val(data.Rows[0].Pjg + "元");
    $("#gdj").val(data.Rows[0].Gjg + "元");
    $("#jdj").val(data.Rows[0].Jjg + "元");

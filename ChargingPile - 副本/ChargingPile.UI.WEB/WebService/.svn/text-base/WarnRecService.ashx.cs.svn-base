    ...
    public class WarnRecService : IHttpHandler, IRequiresSessionState
    {
        ...
        private string FindByTelesignallingWarn(HttpContext context)
        {
            ...
            try
            {
                ...
                message.Status = warnrecbll.FindByTelesignallingWarnCount();
                ret = _jss.Serialize(message);
                ret = ret.Replace("Total", "total");
                ret = ret.Replace("Rows", "rows");
                ret = ret.Replace("\"Status", "\"status");
            }
            catch (Exception e)
            {
                Log.Error(e);
        ...
        private string FindByCardWarn(HttpContext context)
        {
            ...
            try
            {
                ...
                message.Rows = list;
                message.Total = dt.Rows.Count;
                message.Status = warnrecbll.FindByCardWarnCount();
                ret = _jss.Serialize(message);
                ret = ret.Replace("Total", "total");
                ret = ret.Replace("Rows", "rows");
                ret = ret.Replace("\"Status", "\"status");
        ...
        private string FindByCommunicationWarn(HttpContext context)
        {
            ...
            try
            {
                //保存工号
                var worknum = ((Employer)(context.Session[Constant.LoginUser])).WorkNum;
                var dt = warnrecbll.FindByCommunicationWarn(zhanbh);
                ...
                message.Rows = list;
                message.Total = dt.Rows.Count;
                message.Status = warnrecbll.FindByCommunicationWarnCount();
                ret = _jss.Serialize(message);
                ret = ret.Replace("Total", "total");
                ret = ret.Replace("Rows", "rows");
                ret = ret.Replace("\"Status", "\"status");
        ...
        private string FindByPowerFailure(HttpContext context)
        {
            ...
            try
            {
                ...
                message.Rows = list;
                message.Total = dt.Rows.Count;
                message.Status = warnrecbll.FindByPowerFailureCount();
                ret = _jss.Serialize(message);
                ret = ret.Replace("Total", "total");
                ret = ret.Replace("Rows", "rows");
                ret = ret.Replace("\"Status", "\"status");

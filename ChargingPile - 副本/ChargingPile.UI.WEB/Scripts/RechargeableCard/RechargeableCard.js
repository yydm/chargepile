...
function termsearch() {
    ...
    $("#t_rechargeablecard").datagrid("load", { action: "getListQuery", cardno: cardno, sdt: sdt, edt: edt, zhanid: zhanid });
}
...
function OutExcel() {
    ...
    var edt = $("#edt").val();
    var zhanid = $("#ZHANMC").val();
    window.open("/WebService/RequestHandling.ashx?action=CardQueryOutExcel&cardno=" + cardno + "&sdt=" + sdt + "&edt=" + edt + "&zhanid="+ zhanid +"&r=" + Math.random());
}
//加载充电站下拉菜单
function getStation() {
    $.ajax({

...
$(function () {
    ...
    if (offset1 > 0) {
        ...
        data = newsidinfo.split("=")[1]; //对获得的参数字符串按照“=”
        if (data == "time") {
    ...
    $("#t_cardwarn").datagrid({
        ...
        queryParams: { action: "getcardwarn", zhanbh: zhanbh, yxbh: yxbh, processtype: protype, begintime: begintime, endtime: endtime },
        fit: true,
        nowrap: false,
        fitColumns: true,
        striped: true,
        ...
        columns: [[
                    ...
                    { field: "Occurdt", title: "时间", width: 150, align: 'center',
                    ...
                    { field: "ZHANMC", title: "充电场站名称", width: 200, align: 'center' },
                    { field: "YUNXING_BH", title: "桩运行编号", width: 100, align: 'center' },
                    { field: "TARGETDATAKEY", title: "卡号", width: 100, align: 'center' },
                    { field: 'PP', title: '已处理', width: 50, align: 'center',
                        formatter: function (value, src) {
                            var id = src.Id;
                    ...
                    { field: "ProcessFlag", title: "处理方式", width: 150, align: 'center',
                        formatter: function (value, src) {
                            if (value == "0") {
                                return "等待灭警";
                            } else if (value == "1") {
        ...
        onLoadSuccess: function () {
            $($('#t_cardwarn').datagrid("getPanel")).find('a.easyui-linkbutton').linkbutton();
        }

    });
    var hiddenp = $("#h_hidden").val();
    if (hiddenp == "hidden") { $("#t_cardwarn").datagrid("hideColumn", "P"); }
...
function search() {
    ...
    var begintime = $("#begintime").val();
    var endtime = $("#endtime").val();
    $("#t_cardwarn").datagrid("load", { action: "getcardwarn", zhanbh: zhanbh, yxbh: yxbh, processtype: protype, begintime: begintime, endtime: endtime });
    var hiddenp = $("#h_hidden").val();
    if (hiddenp == "hidden") { $("#t_cardwarn").datagrid("hideColumn", "P"); }
}

...
var option_chargpileprice = {
    ...
    border: true,
    striped: true,
    queryParams: { action: 'getchargpileprice', csid: '' },
    url: '../../webservice/PriceAdjustmentService.ashx',
    columns: [[
...
function btnAdd_click() {
    $('#dlg').dialog('open').dialog('setTitle', '添加远程调价计划');
...
function bindlable() {
    ...
    for (var i = 0; i < data.rows.length; i++) {
        switch (data.rows[i].NAME) {
            case "峰电价":
                $("#lblf").val(data.rows[i].PRICE);
                $("#txtf").val(data.rows[i].PRICE);
                break;
            case "谷电价":
                $("#lblg").val(data.rows[i].PRICE);
                $("#txtg").val(data.rows[i].PRICE);
                break;
            case "平电价":
                $("#lblp").val(data.rows[i].PRICE);
                $("#txtp").val(data.rows[i].PRICE);
                break;
            case "尖电价":
                $("#lblj").val(data.rows[i].PRICE);
                $("#txtj").val(data.rows[i].PRICE);
                break;
...
function chargstation_onchanged() {
    $('#dg_schedule_price').datagrid('load', {
        action: 'getchargpileprice',
...
function btnSave_click() {
    var zid = "";
    var selectrows = $("#dg_schedule_price").datagrid("getChecked");
...
function initScheduleTable() {
    $('#dg_schedule').datagrid({
        ...
        rownumbers: true,
        title: "远控远调>>远程调价",
        queryParams: { action: 'inittable', cmdtype: 'AdjustPrice' },
        url: '../../webservice/PriceAdjustmentService.ashx',
        columns: [[
...
function initSchedulePriceTable() {
    $('#dg_schedule_price').datagrid(option_chargpileprice);
}
//----------------------------------------------------------------------------------------------------

...
namespace ChargingPile.BLL
{
    public class RequestHandling_BLL
    {
        ...
        /// 充电站数据查询导出Excel
        /// </summary>
        public DataTable CardQueryOutExcel(string cardno, string zhuangid, DateTime sdt, DateTime edt, string zhanid)
        {
            string strSql = @"      
                            SELECT 
            ...
            if (!string.IsNullOrEmpty(zhuangid))
            {
                strWhere += " and dev_chargpile.yunxing_bh={" + ++i + "} ";
                list.Add(zhuangid);
            }
        ...
        /// 充电信息查询
        /// </summary>
        public DataTable GetDataTableQuery(string cardno, DateTime sdt, DateTime edt,string zhanid,string zhuangid, int page, int rows, ref int total)
        {
            string strSql = @"      
                            SELECT 
                            dev_chargstation.zhuan_mc,

...
namespace ChargingPile.BLL
{
    public class ChargRecordBll : BaseBll<ChargRecordDal>
    {
        ...
        {
            return _chargRecordDal.FindByStandbyInfo(zhuanid);
        }
        /// <summary>
        /// 查询充电桩充电信息
        /// </summary>
        ...
        public string FindByChargingPileZsc(int year, int month)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    TimeSpan ts = new TimeSpan(long.Parse(dt.Rows[0]["s"].ToString()) * 10);
                    //var ts = new TimeSpan(0, 0, Int64.Parse("13532812258235"));

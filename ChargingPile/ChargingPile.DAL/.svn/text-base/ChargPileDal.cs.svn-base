...
namespace ChargingPile.DAL
{
    public class ChargPileDal : BaseDal<ChargPile>
    {
        ...
        public DataTable FindByChargePileDetailInfo(int zhuanid)
        {
            ...
            sql.Append("(select price from (select gatitemid,m_value/1000 as price from gat_data_yc t where t.gatdt in(select max(gatdt) from gat_data_yc t where t.gatitemid= 'FengD_J' ) and t.gatitemid= 'FengD_J') F) fjg,");
            sql.Append("(select price from (select gatitemid,m_value/1000 as price from gat_data_yc t where t.gatdt in(select max(gatdt) from gat_data_yc t where t.gatitemid= 'GuD_J' ) and t.gatitemid= 'GuD_J') G) gjg,");
            sql.Append("(select price from (select gatitemid,m_value/1000 as price from gat_data_yc t where t.gatdt in(select max(gatdt) from gat_data_yc t where t.gatitemid= 'PingD_J' ) and t.gatitemid= 'PingD_J') P) pjg,");
            sql.Append("(select price from (select gatitemid,m_value/1000 as price from gat_data_yc t where t.gatdt in(select max(gatdt) from gat_data_yc t where t.gatitemid= 'JianD_J' ) and t.gatitemid= 'JianD_J') J) jjg,");

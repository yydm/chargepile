...
namespace ChargingPile.DAL
{
    public class WarnDetailDal : BaseDal<WarnDetail>
    {
        ...
        public DataTable FindByEmail()
        {
            Log.Debug("Query方法参数：");
            var sql = new StringBuilder();
            sql.Append("select gwd.id,gwd.warntype title, gwd.warncontext body,gwd.address,gwd.warntype type,gwd.warnrecid ");
            sql.Append("from gat_warndetail gwd  ");
            sql.Append("where upper(gwd.warntype)='Email' and gwd.processflag='0' and rownum<=10");
        ...
        public DataTable FindBySms()
        {
            Log.Debug("Query方法参数：");
            var sql = new StringBuilder();
            sql.Append("select gwd.Id,gwd.WARNRECID,gwd.WARNCONTEXT,gwd.address ");
            sql.Append("from gat_warndetail gwd ");
            sql.Append("where gwd.warntype='SMS' and gwd.processflag='0' and rownum<=10");
            return Oop.GetDataTable(sql.ToString());
        }
        ...
        {
            var sql = @"select t.zhan_jc from dev_chargstation t where t.zhan_bh=(
select substr(dcs.dev_chargpile,0,3) from gat_warndetail gwd  inner join oth_warnrec owr 
on gwd.warnrecid=owr.id inner join dev_chargpile dcs on owr.targetdev=dcs.dev_chargpile 
where owr.targettype='000' and gwd.warnrecid={0})";
            return Oop.GetDataTable(sql, warnid);
        }
        ...
        {
            var sql = @"select dcs.yunxing_bh from gat_warndetail gwd  inner join oth_warnrec owr 
on gwd.warnrecid=owr.id inner join dev_chargpile dcs on owr.targetdev=dcs.dev_chargpile 
where owr.targettype='000' and gwd.warnrecid={0}";
            return Oop.GetDataTable(sql, warnid);
        }
        ...
        public DataTable FindItemName(string warnid)
        {
            ...
            sql.Append("where gwd.warnrecid={0} ");

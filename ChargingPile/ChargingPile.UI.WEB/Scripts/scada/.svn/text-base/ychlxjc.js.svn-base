...
$(function () {
    $("#t_yctbcl").datagrid({
        ...
        columns: [[
            ...
            { field: "EFF_MIN", title: "有效值最小值", align: "center", width: 90,
                formatter: function (val, src) {
                    if (null == val || (val + "").length == 0) {
                        return "—";
                    } else {
            ...
            { field: "EFF_MAX", title: "有效值最大值", align: "center", width: 90,
                formatter: function (val, src) {
                    if (null == val || (val + "").length == 0) {
                        return "—";
                    } else {
...
function InvokeService(page, rows) {
    var req = {
        ...
        ["select * from (select rownum rn, count(*) over() DATACNT,ow.id, " +
            ...
            "and ow.logtype=#logtype order by ow.createdt desc) abc where rn  between "
...
function showHistory() {
    location.href = "ychlxHistoryDetail.htm";
}

    ...
    public class RequestHandling : IHttpHandler, IRequiresSessionState
    {
        ...
        private void CardQueryOutExcel(HttpContext context)
        {
            try
            {
                ...
                try
                {
                    zhaungid = context.Request["zhaungid"].ToString();
                }
                catch (Exception ex)
                {
                    zhaungid = "";
                ...
                RequestHandling_BLL gdrBll = new RequestHandling_BLL();
                DataTable dt = gdrBll.CardQueryOutExcel(cardno,zhaungid, sdt, edt,zhanid);
                if (dt.Rows.Count > 0)
                {
                    DataTableExcel(context,dt, "查询导出" + DateTime.Now.ToString("yyyy年MM月dd日HH时mm分ss秒"));
                }
        ...
        private void GetListQuery(HttpContext context)
        {
            try
            {
                ...
                try
                {
                    zhuangid = context.Request["zhuangid"].ToString();
                }
                catch (Exception ex)
                {
                    zhuangid = "";
                ...
                RequestHandling_BLL gdrBll = new RequestHandling_BLL();
                DataTable dt = gdrBll.GetDataTableQuery(cardno, sdt, edt, zhanid, zhuangid, page, rows, ref total);
